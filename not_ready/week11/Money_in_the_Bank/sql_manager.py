...
from create_tables import Clients, LoginAttempts, BlockedUsers
...
def get_id_by_username(username):
    result = session.query(Clients).filter(Clients.username == username).one()
...
def change_message(new_message, logged_user):
    update_sql = session.query(Clients).update({"message": new_message}).\
                            filter(Clients.id == logged_user.get_id())
